...
def append_line():
    if "data" in request.files:
        d_file = request.files["data"]
        with open(infile, 'a') as append:
            lines_to_write = [l.decode("utf-8") for l in d_file.readlines() if l and (not "MN" in l.decode("utf-8"))]
            if len(lines_to_write) > 0:
                append.write("\n")
                append.writelines(lines_to_write) # dodge header
        processor.proccess_data(infile, "output.csv")
        reload_js()
    return "", 200
