...
The conditional operators avaliable in Beakscript are `=` equals, `!` not equals, `>` greater than, `<` less than, `>=` greater or equal, and `<=` less or equal. Additionally, the `&` and and `|` or operators can be used to compine expressions, and the boolean NOT operator, represented by `~` to avoid confusion with not equal, can be used to negate boolean expressions (`~(5 > 3)` equals 0.0, which represents False)
