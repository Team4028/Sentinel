...
if os.name == "posix":
    ...
    logger.warning(
        "What in the jython is this operating system? probably linux but the bootloader is deleted"
    )
    serial_port = "/dev/ttyUSB0"
local = None
...
def send_mesh_test(message: str):
    ...
    packet = {
        "to": 0,
        "from": 1,
        "decoded": {"portnum": "TEXT_MESSAGE_APP", "payload": message.encode("utf-8")},
    }
    pub.sendMessage(
        "meshtastic.receive", packet=packet, interface=None
    )  # 'recieve' the payload
...
def main(f):
    ...
    pub.subscribe(
        on_receive_wrapper, "meshtastic.receive"
    )  # bind the listener to the recieve channel
    try:
