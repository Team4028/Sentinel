...
class TestingInfo:
    ...
    def apply(self, initial_config):
        """Configues the app config data in memory to the testing info input"""
        initial_config["YEAR"] = self.year


testing_init_info = None
initial_process = True
skip_last_opr_fetching_for_testing = False
...
if testing_init_info != None:
    testing_init_info.apply(app.config)

if testing_init_info == None:  # prevent authentication when unit testing
...
def render_template_style(
    template: str | Template | list[str | Template], **context: Any
) -> str:
...
async def do_data_processing() -> None:
...
# =======================================================
processor = Processor(
    skip_last_opr_fetching_for_testing,
    auth_key,
    (
...
if testing_init_info != None:
    processor.event_key = testing_init_info.event_key
...
def compile_scouting_dashboard(url: str) -> None:
...
def send_generic_notification(data: dict) -> None:
...
def send_change_notification(lines: str | None = None) -> None:
    """Sends a notification.

    If 'lines' is None, this will send a notification letting the user know that new changes are avaliable (restrict append level is 2) with a link to :func:`changes`.

    If 'lines' is not None, restrict append level is assumed to be 1 and a new notification that's body has ``lines`` and a link to remove ``lines`` from the input data will be sent.
    """
    data = {
...
def rm_row_hash(hashes: list[str]) -> None:
...
def save_photo(filestorage, team: str) -> None:
...
def handle_mesh_line(message: str) -> None:
    """This function is a consumer for a meshtastic message (line of csv) and will append it to the csv much like :func:`append_lines`"""
...
def mesh_listen() -> None:
    """Binds :func:`handle_mesh_line` to the meshtastic port (listens for meshages)"""
...
def handle_exception(e: Exception) -> str | tuple[str, Literal[500]]:
...
def handle_401(e: Exception) -> str | tuple[str, Literal[401]]:
...
def handle_403(e: Exception) -> str | tuple[Literal["Error: restricted"], Literal[403]]:
...
def handle_404(
    e: Exception,
) -> str | tuple[Literal["Error: page not found"], Literal[404]]:
...
def notify_q() -> (
    Response
    | tuple[Literal["Error, invalid cid"], Literal[400]]
    | tuple[Literal["No notifications in queue"], Literal[204]]
):
...
def login() -> (
    tuple[Literal["Login Successful"], Literal[200]]
    | tuple[Literal["Invalid Credentials"], Literal[401]]
    | str
):
    ...
    return render_template_style(
        "login.html", form=form, allow_create=app.config["ALLOW_CREATE_ACCOUNT"]
    )


@app.route("/create-account", methods=["GET", "POST"])
def create_account() -> (
    tuple[Literal["Account created"], Literal[200]]
    | str
    | tuple[Literal["Error: restricted"], Literal[403]]
):
    ...
    if app.config["ALLOW_CREATE_ACCOUNT"] or auth.get_user_is_admin(current_user):
        ...
        return render_template_style(
            ...
            form=form,
            already_logged_in=(
                0 if isinstance(current_user, AnonymousUserMixin) else 1
            ),
        )
    else:
        if request.method == "GET":
...
def delete_account() -> (
    tuple[Literal["Account deleted."], Literal[200]]
    | tuple[Literal["Deletion prohibited"], Literal[403]]
):
...
def manage_accounts() -> str:
    """page for viewing and deleting accounts"""
    return render_template_style(
        "manage-accounts.html",
        headers=json.dumps(auth.LOGIN_TABLE_HEADERS),
        inp_data=json.dumps([",".join(map(str, x)) for x in auth.get_users()]),
    )


@app.post("/create-login")
def create_login() -> (
    tuple[Literal["Success"], Literal[200]] | tuple[str, Literal[500]]
):
...
@app.post("/login-ovr")
def login_override() -> (
    tuple[Literal["Login Override Successful"], Literal[200]]
    | tuple[Literal["Invalid Credentials"], Literal[401]]
):
...
@app.get("/edit-file")
def edit_file() -> str | tuple[Literal["Error, path does not exist"], Literal[400]]:
...
@app.get("/view-file")
def view_file() -> (
    Response | tuple[Literal["Error, path does not exist"], Literal[400]]
):
...
def rename_file() -> (
    tuple[Literal["Not allowed in local testing"], Literal[401]]
    | tuple[Literal["Success"], Literal[200]]
    | tuple[Literal["Error, path does not exist"], Literal[400]]
):
...
@app.get("/我是谁")
def whoami() -> Response:
    """Returns a dictionary specifying the logged_in status of the user, and un/id/isadmin if applicable"""
...
@app.get("/health")
def health() -> (
    tuple[Literal["No event data"], Literal[200]]
    | tuple[Literal["Sentinel is watching"], Literal[200]]
):
...
@app.get("/percent")
def percent() -> Response | str:  # need because grafana infinity can't do local JSON
...
@app.post("/upload")
def upload_file() -> tuple[Literal[""], Literal[200]] | tuple[str, Literal[500]]:
...
@app.post("/upload-alt")
def upload_other_files() -> tuple[Literal[""], Literal[200]] | tuple[str, Literal[500]]:
...
def upload_auto() -> (
    tuple[Literal["Saved!"], Literal[200]] | tuple[str, Literal[500]] | None
):
...
@app.post("/reproc")
def reprocess() -> (
    tuple[Literal["Data reloaded."], Literal[200]] | tuple[str, Literal[500]]
):
...
def run_process() -> (
    tuple[Literal["Error, invalid process requested"], Literal[400]]
    | tuple[Literal["Process requested"], Literal[200]]
...
def get_team_pics() -> (
    Response
    | tuple[Literal["Error, team picture not found"], Literal[400]]
    | tuple[str, Literal[500]]
    | tuple[Literal["Error: invalid request"], Literal[400]]
):
...
def get_team_indicies() -> (
    ...
    if "team" in request.args:
        try:
            max_idx = len(
                list(Path("photos").glob(f"{request.args.get("team", 0).strip()}*.*"))
            )
...
@app.get("/next-3")
def n3() -> Response:
    """Returns the next :code:`app.config["NEXT_N_MATCHES_NUMBER"]` (currently 3, hence the name) matches after ``?mkey`` that contain ``?team``
...
def delete_change(
    idx: int,
) -> tuple[Literal[""], Literal[200]] | tuple[Literal["Invalid request"], Literal[400]]:
...
def delete_lines() -> tuple[Literal[""], Literal[200]]:
    """For restrict append level 1: deletes the already applied append cooresponding to the input json's `lines` field using :func:`rm_row_hash`"""
...
def teams_in_match() -> (
    Response | tuple[Literal["Error, match not in data"], Literal[404]]
):
...
def load_event_data() -> (
    tuple[Literal[""], Literal[200]]
    | tuple[str, Literal[500]]
    | tuple[Literal["Invalid event key"], Literal[400]]
):
...
@app.post("/dash-reset")
def reset_dash() -> tuple[Literal[""], Literal[200]] | tuple[str, Literal[500]]:
...
def clear_datain() -> tuple[Literal[""], Literal[200]] | tuple[str, Literal[500]]:
...
@app.post("/clear-db")
def clear_db() -> tuple[Literal[""], Literal[200]] | tuple[str, Literal[500]]:
...
def test_notification() -> tuple[Literal["Sent"], Literal[200]]:
...
def test_tba_key() -> (
    tuple[Literal["true"], Literal[200]] | tuple[Literal["false"], Literal[200]]
):
...
def set_tba_key() -> (
    tuple[Literal["Bad TBA key"], Literal[400]]
    | tuple[Literal[""], Literal[200]]
    | tuple[str, Literal[500]]
):
...
@app.post("/set-creds")
def set_creds() -> tuple[Literal["Success"], Literal[200]] | tuple[str, Literal[500]]:
...
def calc_team_score() -> Response:
    ...
    for team in teams:
        if not apputils.can_cast(team, int):
            continue
...
def view_picklists() -> str:
    """Returns an html page displaying all of the submitted picklists"""
    return render_template_style(
...
def get_picklist() -> Response | tuple[Literal["File not found"], Literal[400]]:
    """Returns a dictionary representing the picklist for the specified user"""
    jsfiles = list(
        Path("picklists").glob(f"{request.headers.get('name', current_user.id)}.json")
    )
    if len(jsfiles) > 0:
        jsfile = jsfiles[0]
...
def save_picklist() -> tuple[Literal["Success"], Literal[200]]:
    ...
    for _list in js.keys():
        ...
        for team in js[_list]:
            ...
            if _list in current_data:
                if (
                    idx := next(
                        (
                            i
                            for i, x in enumerate(current_data[_list])
                            if x["team"] == team
                        ),
                        None,
                    )
            ...
            teams_new.append(
                {"team": team, "like": likes, "dlike": dlikes, "comments": cm}
            )
        js[_list] = teams_new
...
@app.get("/download")
def dload() -> tuple[Literal["File not found."], Literal[404]] | Response:
...
def upload_folder() -> (
    tuple[Literal["Error, no selected file"], Literal[400]]
    | tuple[Literal["Invalid Zip file"], Literal[400]]
    | tuple[Literal["File uploaded successfully"], Literal[200]]
):
    """
